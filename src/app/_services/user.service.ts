...
export class UserService {
    ...
    habilitar(user: User): Observable<User> {
        const url = environment.api_url + 'usuario/'.concat(user.id.toString(), '/habilitar');
    ...
    deshabilitar(user: User): Observable<User> {
        const url = environment.api_url + 'usuario/'.concat(user.id.toString(), '/deshabilitar');
        return this.http.put<User>(url, httpOptions);
    }

    /** GET Rutas Agregadas by the User from the database */
    listAllRutasAgregadas(user: User): Observable<Ruta[]> {
        return this.http.get<Ruta[]>(environment.api_url + 'usuario/'.concat(user.id.toString(), '/rutasAgregadas'));
    }

    /** GET Rutas Recorridas by the User from the database */
    listAllRutasRecorridas(user: User): Observable<Ruta[]> {
        return this.http.get<Ruta[]>(environment.api_url + 'usuario/'.concat(user.id.toString(), '/rutasRecorridas'));
    }

    /** GET Rutas Descubrir by the User from the database */
    listAllRutasDescubrir(user: User): Observable<Ruta[]> {
        return this.http.get<Ruta[]>(environment.api_url + 'usuario/'.concat(user.id.toString(), '/rutasDescubrir'));
    }
